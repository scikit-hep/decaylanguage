...
def ampgen2goofit(filename, ret_output=False):
    ...
    # Never embed ANSI escape codes in the returned/generated output; only
    # colorize when printing to a real terminal.
    def paint(style, text):
        return text if ret_output else (style | text)

    lines, all_states = GooFitChain.read_ampgen(str(filename))

    printer(r"/* Autogenerated file by AmpGen2GooFit")
    printer("Generated on ", datetime.datetime.now(tz=datetime.timezone.utc))

    printer("\n")
    for seen_factor in {p.spindetails() for p in lines}:
        my_lines = [p for p in lines if p.spindetails() == seen_factor]
        printer(paint(colors.bold, seen_factor), ":", *my_lines[0].spinfactors)
        for line in my_lines:
            printer(" ", paint(colors.blue, str(line)))
    ...
    printer(
        paint(colors.bold & colors.green, "\n\nAll discovered spin configurations:")
    )

    for line in sorted({iline.spindetails() for iline in lines}):
        printer(paint(colors.green, line))

    printer(paint(colors.bold & colors.blue, "\n\nAll known spin configurations:"))

    # TODO: 4 body only
    for e in SF_4Body:
        printer(paint(colors.blue, e.name))
...
def ampgen2goofitpy(filename, ret_output=False):
    ...
    for n, line in enumerate(lines):
        printer("# Line", n)
        printer(line.to_goofit(all_states[1:]), end="\n\n\n")
    printer("DK3P_DI.amplitudes = amplitudes_list")
